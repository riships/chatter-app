...
export const getDirectRoomId = (userA: string, userB: string): string => {
...
const broadcastAllUsers = async (
  ...
  try {
    ...
    const userAccounts: UserAccount[] = users.map((u) => ({
      username: u.username,
...
const sendUserJoinedRooms = async (socket: CustomSocket, username: string) => {
  try {
    const rooms = await Room.find({ members: username }).sort({ createdAt: -1 });
    const roomPayloads: RoomData[] = rooms.map((r) => ({
      roomId: r.roomId,
      name: r.name,
...
const refreshJoinedRoomsForMembers = async (
  io: Server<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>,
  members: string[]
) => {
  try {
...
export const registerChatSocketHandlers = (
  ...
  // Event: 'user'
  socket.on('user', async (username: string) => {
  ...
  socket.on('update-group-info', async (data: { roomId: string; name: string; description?: string }) => {
    const { user } = socket.data;
    if (!user || !data.roomId || !data.name) return;
    ...
    try {
      ...
      const updateSysMessage: UserDetails = {
        user_type: 'System',
        user: user,
  ...
  socket.on('remove-group-member', async (data: { roomId: string; targetUser: string }) => {
    const { user } = socket.data;
    if (!user || !data.roomId || !data.targetUser) return;
    ...
    try {
      ...
      const removeMessage: UserDetails = {
        user_type: 'System',
        user: user,
  ...
  socket.on('leave-group-room', async (data: { roomId: string }) => {
    const { user } = socket.data;
    ...
    try {
      ...
      if (roomDoc) {
        ...
        const leaveMessage: UserDetails = {
          user_type: 'System',
          user: user,
  ...
  socket.on('delete-group-room', async (data: { roomId: string }) => {
    const { user } = socket.data;
    if (!user || !data.roomId) return;
  ...
  socket.on('create', (roomName: string) => {
    socket.data.room = roomName;
    handleRoomJoin();
  });

  // Event: 'start-direct-chat'
  socket.on('start-direct-chat', async (data: { targetUser: string }) => {
    const { user } = socket.data;
    if (!user || !data.targetUser) return;
  ...
  // Event: 'send-direct'
  socket.on('send-direct', async (data: { targetUser: string; text: string }) => {
    const { user, imgUrl } = socket.data;
    if (!user || !data.targetUser || !data.text) return;
  ...
  // Event: 'send'
  socket.on('send', async (msg: string) => {
    const { user, room, imgUrl } = socket.data;
    if (!msg || !user || !room) return;
  ...
  // Event: 'img-url'
  socket.on('img-url', async (imgUrl: string) => {
  ...
  // Event: 'sendStatus'
  socket.on('sendStatus', (msg: string | null | undefined) => {
